 ...
 *
 * v6.0.0 (P0-4A): Tauri 环境下密文落本地文件（app_data_dir/frosthold/），
 * 不再进 localStorage（WebView2 可能被系统清理）。Web 环境保持原行为。
 *
 * 工作原理：
 * 1. 首次使用生成 AES-GCM 密钥并存储在 IndexedDB（不可提取）
 * 2. 加密时生成随机 IV，与密文拼接后 Base64 编码
 * 3. 密文存入 localStorage / Tauri 文件，密钥本身不出存储
 *
 * 安全特性：
...
import { TauriAdapter, isTauriEnv } from '../storage/tauri-adapter';
...
let tauriAdapterCache: TauriAdapter | null = null;

function getTauriAdapter(): TauriAdapter | null {
  if (!isTauriEnv()) return null;
  if (!tauriAdapterCache) {
    try { tauriAdapterCache = new TauriAdapter(); } catch { return null; }
  }
  return tauriAdapterCache;
}

/** Tauri 下读写密文键（键名做安全处理：ai-narrator-sec-deepseek-api-key → slot 白名单外，改用 config 文件内嵌） */
async function tauriReadSecret(key: string): Promise<string | null> {
  const adapter = getTauriAdapter();
  if (!adapter) return null;
  try {
    const data = await adapter.get<Record<string, string>>('config');
    return data?.[key] ?? null;
  } catch { return null; }
}

async function tauriWriteSecret(key: string, value: string): Promise<void> {
  const adapter = getTauriAdapter();
  if (!adapter) return;
  try {
    const data = (await adapter.get<Record<string, string>>('config')) ?? {};
    data[key] = value;
    await adapter.set('config', data);
  } catch { /* ignore */ }
}

async function tauriRemoveSecret(key: string): Promise<void> {
  const adapter = getTauriAdapter();
  if (!adapter) return;
  try {
    const data = (await adapter.get<Record<string, string>>('config')) ?? {};
...
export async function secureSet(key: string, value: string): Promise<boolean> {
  try {
    ...
    // v6.0.0 (P0-4A): Tauri 环境密文落本地文件（config.json 内嵌）
    if (isTauriEnv()) {
      await tauriWriteSecret(LS_PREFIX + key, encoded);
      return true;
    }
...
export async function secureGet(key: string): Promise<string | null> {
  try {
    ...
    // v6.0.0 (P0-4A): Tauri 密文从本地文件读
    let encoded: string | null = null;
    if (isTauriEnv()) {
      encoded = await tauriReadSecret(LS_PREFIX + key);
    } else {
      encoded = localStorage.getItem(LS_PREFIX + key);
    }
...
export function secureRemove(key: string): void {
  try {
    if (isTauriEnv()) {
      void tauriRemoveSecret(LS_PREFIX + key);
      return;
    }
