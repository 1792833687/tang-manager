...
import { TauriAdapter, isTauriEnv } from './tauri-adapter';

/**
 * 存储路由 — 四级降级：Tauri(桌面文件) → IndexedDB → localStorage → Memory
 *
 * v6.0.0 (P0-3): Tauri 环境下优先用本地文件（不受 WebView2 清理影响），
 * Web 环境保持原有降级链。
 *
 * 用法：
 * ```ts
...
export type StorageBackend = 'tauri' | 'indexeddb' | 'localstorage' | 'memory';
...
export class StorageRouter {
  ...
  static async getAdapter(): Promise<IStorageAdapter> {
    ...
    // 0. Tauri 桌面环境 → 本地文件（P0-3）
    if (isTauriEnv()) {
      const tauri = new TauriAdapter();
      if (await tauri.isAvailable()) {
        this.adapter = tauri;
        this.backend = 'tauri';
        return this.adapter;
      }
      console.warn('[StorageRouter] Tauri 存储不可用，降级到 IndexedDB');
    }
