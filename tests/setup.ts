...
// 提取说明：源 setup.ts 另导入 ../src/systems/map/types（Region/Tile/TileCoord/MapState）并提供
// createTestCoord/Tile/Region/RegionGrid/MapState 地图夹具 —— 这些为凛冬要塞地图系统专属，
// 唐朝掌柜测试不引用（grep 核对），独立项目不迁移地图系统，故一并移除。
...
// 提取说明：源文件此处原为 createTestCoord / createTestTile / createTestRegion /
// createTestRegionGrid / createTestMapState（地图系统夹具），已随地图系统一并移除。
